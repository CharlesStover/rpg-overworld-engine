var $document = $(document),
	...
	GAMEngine = {
		...
		// functions for jQuery objects
		$: {

			// X-coordinate sprite sheet navigation
			backgroundPositionX: function(x)
			{
				return this.css("background-position", this[0].style.backgroundPosition.replace(/^[\d\-]+(?:em|%|pt|px)?/, x));
			},

			// XY-coordinates sprite sheet navigation
			backgroundPositionXY: function(x, y)
			{
			...
			backgroundPositionY: function(y)
			{
				return this.css("background-position", this[0].style.backgroundPosition.replace(/[\d\-]+(?:em|%|pt|px)?$/, y));
			},
			...
			safe: function(x, y)
			{
				...
				// Only calculate if we're moving.
				if (
					x != 0 ||
					y != 0
				)
				{
					...
					// Round X and Y "up" for fractions of a pixel.
					var ceiling = [
						...
						collided = [x == 0 ? 0 : false, y == 0 ? 0 : false],

						// The object ID of this object.
						id = this.data("id"),

						// The final values we will be able to shift.
						shift = [
							[0, 0],
							[0, 0]
						],
					...
					// Determine which axis's distance is larger.
					var max = absolute[1] > absolute[0] ? 1 : 0;
					var maxValue = absolute[max];

					// For every pixel there is to travel,
					for (a = 1; a <= maxValue; a++)
					{

						// for each object
						for (b = 0; b < GAMEngine.$objects.length; b++)
						{
							...
							if (b != id - 1)
							{
								...
								if (GAMEngine.$objects[b].data("collisionable"))
								{
									...
									shift = [
											...
											// the last position
											[
												collided[0] === false ? (a == 1 ? 0 : (max == 0 ? (ceiling[0] > 0 ? a - 1 : -1 * (a - 1)) : Math.round(x / absolute[1] * (a - 1)))) : shift[1][0],
												collided[1] === false ? (a == 1 ? 0 : (max == 1 ? (ceiling[1] > 0 ? a - 1 : -1 * (a - 1)) : Math.round(y / absolute[0] * (a - 1)))) : shift[1][1]
											],

											// the new position
											[
												collided[0] === false ? max == 0 ? (ceiling[0] > 0 ? a : -1 * a) : Math.round(x / absolute[1] * a) : shift[0][0],
												collided[1] === false ? max == 1 ? (ceiling[1] > 0 ? a : -1 * a) : Math.round(y / absolute[0] * a) : shift[0][1]
											]
										];
...
*/
									...
									// If the next pixel is occupied.
									if (this.touching(GAMEngine.$objects[b], shift[1][0], shift[1][1]))
									{

										// check to see if it's collided with the X movement
										if (
											collided[0] === false &&
											this.touching(
												GAMEngine.$objects[b],
										...
										// check to see if it's collided with the Y movement
										if (
											collided[1] === false &&
											this.touching(
												GAMEngine.$objects[b],
										...
										// If both the horizontal and vertical movements are open,
										// but the next pixel is occupied, we've hit a corner.
										if (
											collided[0] === false &&
											collided[1] === false
										)
									...
									// If both X and Y have collided, don't bother checking further objects for the same coordinates.
									if (
										collided[0] !== false &&
										collided[1] !== false
									)
					...
					// If it collided after the first pixel, don't go anywhere.
					/*if (a == 0)
			...
			touching: function(x1, y1, x2, y2, shiftx, shifty)
			{
		...
		createObject: function(settings)
		{
			...
			settings = $.extend(
				{}, // empty
				{ // default
					collisionable: true, // can collide with objects
					data: false, // data stored in data()
					gravityX: 0, // horizontal gravitational pull
					gravityY: 0, // vertical gravitational pull
					gravityCallback: false, // function to call every time gravity is applied
					gravityCollisionX: false, // function to call when gravity causes a horizontal collision
					gravityCollisionY: false, // function to call when gravity causes a vertical collision
					group: false, // class name
					left: "0", // CSS left
			...
			var htmlId = "object" + id.toString();
			this.$screen.append('<div class="object' + (settings.group ? " " + settings.group : "") + '" id="' + htmlId + '"></div>');
			var $object = $("#" + htmlId);
			if (settings.data)
				$object.data(settings.data);
			$object.data({
				collisionX: false, // horizontal gravity collision
				collisionY: false, // vertical gravity collision
				collisionable: settings.collisionable,
				gravitiesX: 0, // amount of times horizontal gravity has been applied to an object since last gravity collision
				gravitiesY: 0, // amount of times vertical gravity has been applied to an object since last gravity collision
				gravityX: settings.gravityX,
				gravityY: settings.gravityY,
				id: id,
				movementX: 0, // X counter for safe(), used to determine whether or not a fraction of a pixel should be rounded up or down
				movementY: 0 // Y counter for safe(), used to determine whether or not a fraction of a pixel should be rounded up or down
			});
			if (settings.gravityCallback)
				$object.bind("gravityCallback", settings.gravityCallback);
			if (settings.gravityCollisionX)
				$object.bind("gravityCollisionX", settings.gravityCollisionX);
			if (settings.gravityCollisionY)
				$object.bind("gravityCollisionY", settings.gravityCollisionY);
			delete settings.collisionX, settings.collisionY, settings.collisionable, settings.data, settings.gravityX, settings.gravityY, settings.gravityCallback, settings.gravityCollisionX, settings.gravityCollisionY, settings.group;
			$object.css(settings);
		...
		gravity: function()
		{
			...
			for (var x = 0; x < this.$objects.length; x++)
			{
				...
				if (
					this.$objects[x].data("gravityX") != 0 ||
					this.$objects[x].data("gravityY") != 0
				)
				{
					var move = this.$objects[x].safe(
							this.$objects[x].data("gravityX"),
							this.$objects[x].data("gravityY")
						);
					if (
						move[0] != 0 ||
						move[1] != 0
					...
					{
						this.$objects[x].data("gravitiesX", move[2] ? 0 : this.$objects[x].data("gravitiesX") + 1);
						this.$objects[x].data("gravitiesY", move[3] ? 0 : this.$objects[x].data("gravitiesY") + 1);
						this.$objects[x].move(move[0], move[1]);
						if (this.$objects[x].hasEvent("gravityCallback"))
							this.$objects[x].trigger("gravityCallback");
					}
					else
					{
						this.$objects[x].data("gravitiesX", 0);
						this.$objects[x].data("gravitiesY", 0);
					}
					if (
						move[2] &&
						!this.$objects[x].data("collisionX") &&
						this.$objects[x].hasEvent("gravityCollisionX")
					)
						this.$objects[x].trigger("gravityCollisionX");
					if (
						move[3] &&
						!this.$objects[x].data("collisionY") &&
						this.$objects[x].hasEvent("gravityCollisionY")
					)
						this.$objects[x].trigger("gravityCollisionY");
					this.$objects[x].data("collisionX", move[2]);
					this.$objects[x].data("collisionY", move[3]);
				}

				/*var gravity = {
						x: this.$objects[x].data("gravityX"),
						y: this.$objects[x].data("gravityY")
					};
				...
				)
				{
					...
					var absGravity = {
							x: Math.abs(gravity.x),
							y: Math.abs(gravity.y)
						};

					// Have we collided yet?
					var collided = false,
						maxGravity = absGravity.x > absGravity.y ? "x" : "y";

					// for every pixel it is to travel,
					// (if we already know it collides after Y pixels, don't check further than Y)
					for (var y = 1; y <= Math.ceil(maxGravity == "x" ? absGravity.x : absGravity.y); y++)
					{
						...
						for (var z = 0; z < this.$objects.length; z++)
						{
							...
							if (x != z)
							{
								...
								if (
									...
									this.$objects[x].touching(this.$objects[z], maxGravity == "x" ? gravity.x > 0 ? y : -1 * y : Math.round(gravity.x / y), maxGravity == "y" ? gravity.y > 0 ? y : -1 * y : Math.round(gravity.y / y))
								)
								{
									collided = true;
					...
					if (y > 0)
					{
						...
						this.$objects[x]
							.css("left",
								(
									this.px2int(this.$objects[x].css("left")) +
									(maxGravity == "x" ? gravity.x > 0 ? y : -1 * y : Math.round(gravity.x / y))
								).toString() + "px"
							)
							.css("top",
								(
									...
									(maxGravity == "y" ? gravity.y > 0 ? y : -1 * y : Math.round(gravity.y / y))
								).toString() + "px"
							);
...
// extend the jQuery object with the GAMEngine jQuery object functions
$.extend($.fn, GAMEngine.$);
